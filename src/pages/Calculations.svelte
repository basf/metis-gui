            ...
            <Panel>
                ...
                {#each dataCalc as item, i (item.uuid)}
                    ...
                    <Tile>
                        ...
                        <svelte:fragment slot="subtitle">
                            ...
                            {:else}
                                <small class="tile-subtitle text-gray">Type &bull; {item.type} &bull; {(new Date()).toLocaleTimeString()}</small>
                            {/if}
                        </svelte:fragment>
                        <svelte:fragment slot="action">
                            ...
                            {:else}
                                <IconButton icon="icon-forward" on:click={() => setCalculation(item.uuid)}></IconButton>
                            {/if}
                ...
                <svelte:fragment slot="footer">
                    <div class="columns mb-2 mt-2">
                        <div class="column">
                            <Input rows={4} placeholder="Paste POSCAR, CIF, or Optimade JSON" bind:value={content} />
                        </div>
                    ...
                    <Button variant="primary" block disabled={dataCalc.length >= 5} on:click={addDataItem}>Add structure</Button>
                </svelte:fragment>
            </Panel>
        </div>
    </div>
...
<script>
    ...
    import {
        ...
        getCalculations, setCalculation, delCalculation
    } from '@/services/api';
